   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key) {
      ...
      //set eosio.token contract & initialize it
      {
...

         {
            ...
            act.data = enumivo_token_serializer.variant_to_binary("create", fc::json::from_string("{\"issuer\":\"eosio.token\",\"maximum_supply\":\"1000000000.0000 CUR\", \"can_freeze\":0, \"can_recall\":0, \"can_whitelist\":0}}"));
         ...
         }
         {
            ...
            act.name = N(issue);
            act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
            act.data = enumivo_token_serializer.variant_to_binary("issue", fc::json::from_string("{\"to\":\"eosio.token\",\"quantity\":\"600.0000 CUR\",\"memo\":\"\"}"));
            trx.actions.push_back(act);
         }
         {
            action act;
            ...
            act.name = N(transfer);
            act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
            act.data = enumivo_token_serializer.variant_to_binary("transfer", fc::json::from_string("{\"from\":\"eosio.token\",\"to\":\"txn.test.a\",\"quantity\":\"200.0000 CUR\",\"memo\":\"\"}"));
            trx.actions.push_back(act);
         }
         {
            action act;
            ...
            act.name = N(transfer);
            act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
            act.data = enumivo_token_serializer.variant_to_binary("transfer", fc::json::from_string("{\"from\":\"eosio.token\",\"to\":\"txn.test.b\",\"quantity\":\"200.0000 CUR\",\"memo\":\"\"}"));
            trx.actions.push_back(act);
         }
