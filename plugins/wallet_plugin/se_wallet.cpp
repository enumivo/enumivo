...
static void check_signed() {
   ...
   if(is_valid != errSecSuccess) {
      ...
      ENU_THROW(secure_enclave_exception, "");
   }
}
...
se_wallet::se_wallet() : my(new detail::se_wallet_impl()) {
   ...
   ENU_THROW(secure_enclave_exception, "Secure Enclave not supported on this hardware");
}
...
void se_wallet::lock() {
   ENU_ASSERT(!is_locked(), wallet_locked_exception, "You can not lock an already locked wallet");
   my->locked = true;
}
...
bool se_wallet::remove_key(string key) {
   ENU_ASSERT(!is_locked(), wallet_locked_exception, "You can not remove a key from a locked wallet");
