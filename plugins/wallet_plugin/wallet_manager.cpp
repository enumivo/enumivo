...
void wallet_manager::set_timeout(const std::chrono::seconds& t) {
   ...
   ENU_ASSERT(timeout_time >= now && timeout_time.time_since_epoch().count() > 0, invalid_lock_timeout_exception, "Overflow on timeout_time, specified ${t}, now ${now}, timeout_time ${timeout_time}",
...
std::string wallet_manager::create(const std::string& name) {
   ...
   // This can happen if the wallet file is removed while enuwallet is running.
...
void wallet_manager::open(const std::string& name) {
   ...
   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is added while enuwallet is running.
   auto it = wallets.find(name);
   if (it != wallets.end()) {
      wallets.erase(it);
   }
...
void wallet_manager::own_and_use_wallet(const string& name, std::unique_ptr<wallet_api>&& wallet) {
   if(wallets.find(name) != wallets.end())
      ENU_THROW(wallet_exception, "Tried to use wallet name that already exists.");
   wallets.emplace(name, std::move(wallet));
}
   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(ec != boost::system::error_code()) {
         if(rc.type() == bfs::file_not_found) {
            appbase::app().quit();
            ENU_THROW(wallet_exception, "Lock file removed while enuwallet still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   //This is technically somewhat racy in here -- if multiple enuwallet are in this function at once.
   ...
   lock_path = dir / "wallet.lock";
   {
      ...
      ENU_ASSERT(!x.fail(), wallet_exception, "Failed to open wallet lock file at ${f}", ("f", lock_path.string()));
   }
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      ENU_THROW(wallet_exception, "Failed to lock access to wallet directory; is another enuwallet running?");
   }
