      ...
      private:
         inline string is_clause_decl( string line ) {
            ...
            if ( regex_match( line, match, regex("(###[ ]+CLAUSE[ ]+NAME[ ]*:[ ]*)(.*)", regex_constants::ECMAScript) ) ) {
               ENU_ASSERT( match.size() == 3, invalid_ricardian_clause_exception, "Error, malformed clause declaration" );
               return match[2].str();
            }
            return {};
         }
         ...
         inline string is_action_decl( string line ) {
            ...
            if ( regex_match( line, match, regex("(##[ ]+ACTION[ ]+NAME[ ]*:[ ]*)(.*)", regex_constants::ECMAScript) ) ) {
               ENU_ASSERT( match.size() == 3,  invalid_ricardian_action_exception, "Error, malformed action declaration" );
               return match[2].str();
            }
            return {};
         }
         ...
         void parse_clauses( ifstream& clause_file ) {
            ...
            while ( clause_file.peek() != EOF ) {
               ...
               if ( !(_name = is_clause_decl( line )).empty() ) {
                  if ( !first_time ) {
                     if (body.str().empty() ) {
                        ENU_ASSERT( false, invalid_ricardian_clause_exception, "Error, invalid input in ricardian clauses, no body found" );
                     }
