...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         ENU_ASSERT( head->block && head->block->transactions.size() == head->trxs.size(), block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
...
*/
   ...
   void on_irreversible( const block_state_ptr& s ) {
      ...
      if (!log_head) {
         if (s->block) {
            ENU_ASSERT(s->block_num == blog.first_block_num(), block_log_exception, "block log has no blocks and is appending the wrong first block.  Expected ${expecgted}, but received: ${actual}",
   ...
   void push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         ...
         ENU_ASSERT( s != controller::block_status::incomplete, block_validate_exception, "invalid block status for a completed block" );
   ...
   void push_confirmation( const header_confirmation& c ) {
      ENU_ASSERT(!pending, block_validate_exception, "it is not valid to push a confirmation when there is a pending block");
      fork_db.add( c );
