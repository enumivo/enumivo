...
//   context.require_write_lock( config::enumivo_auth_scope );
   ...
   ENU_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_enumivo_setcode(apply_context& context) {
   ...
   ENU_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   ENU_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   ENU_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
