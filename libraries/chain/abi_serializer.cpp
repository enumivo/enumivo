...
namespace enumivo { namespace chain {
   ...
   void abi_serializer::set_abi(const abi_def& abi, const fc::microseconds& max_serialization_time) {
      ...
      for( const auto& td : abi.types ) {
         ENU_ASSERT(_is_type(td.type, 0, deadline, max_serialization_time), invalid_type_inside_abi, "invalid type ${type}", ("type",td.type));
         ENU_ASSERT(!_is_type(td.new_type_name, 0, deadline, max_serialization_time), duplicate_abi_type_def_exception, "type already exists", ("new_type_name",td.new_type_name));
   ...
   bool abi_serializer::_is_type(const type_name& rtype, size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time)const {
      ENU_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
   ...
   void abi_serializer::validate(const fc::time_point& deadline, const fc::microseconds& max_serialization_time)const {
      ...
      for( const auto& t : typedefs ) { try {
         ENU_ASSERT(_is_type(t.second, 0, deadline, max_serialization_time), invalid_type_inside_abi, "${type}", ("type",t.second) );
      } FC_CAPTURE_AND_RETHROW( (t) ) }
      for( const auto& s : structs ) { try {
         ...
         for( const auto& field : s.second.fields ) { try {
            ENU_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
            ENU_ASSERT(_is_type(_remove_bin_extension(field.type), 0, deadline, max_serialization_time), invalid_type_inside_abi, "${type}", ("type",field.type) );
         } FC_CAPTURE_AND_RETHROW( (field) ) }
      } FC_CAPTURE_AND_RETHROW( (s) ) }
      for( const auto& s : variants ) { try {
         for( const auto& type : s.second.types ) { try {
            ENU_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
            ENU_ASSERT(_is_type(type, 0, deadline, max_serialization_time), invalid_type_inside_abi, "${type}", ("type",type) );
         } FC_CAPTURE_AND_RETHROW( (type) ) }
      } FC_CAPTURE_AND_RETHROW( (s) ) }
      for( const auto& a : actions ) { try {
        ENU_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
        ENU_ASSERT(_is_type(a.second, 0, deadline, max_serialization_time), invalid_type_inside_abi, "${type}", ("type",a.second) );
   ...
   {
      ENU_ASSERT( ++recursion_depth < max_recursion_depth, abi_recursion_depth_exception, "recursive definition, max_recursion_depth ${r} ", ("r", max_recursion_depth) );
      ENU_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
                                                   ...
                                                   size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time )const
   {
      ...
      if ( is_array(rtype) ) {
        ...
        for( decltype(size.value) i = 0; i < size; ++i ) {
           ...
           ENU_ASSERT( !v.is_null(), unpack_exception, "Invalid packed array" );
        ...
        return flag ? _binary_to_variant(ftype, stream, recursion_depth, deadline, max_serialization_time) : fc::variant();
         ...
         if( v != variants.end() ) {
            ...
            ENU_ASSERT( (size_t)select < v->second.types.size(), unpack_exception, "Invalid packed variant" );
      ...
      ENU_ASSERT( mvo.size() > 0, unpack_exception, "Unable to unpack stream ${type}", ("type", type) );
   ...
   { try {
      ...
      if( btype != built_in_types.end() ) {
         ...
         ENU_ASSERT( var.is_array() && var.size() == 2 && var[size_t(0)].is_string(), abi_exception, "expected array containing variant" );
         auto& v = variants.find(rtype)->second;
         auto it = find(v.types.begin(), v.types.end(), var[size_t(0)].get_string());
         ENU_ASSERT( it != v.types.end(), abi_exception, "type is not valid within this variant" );
            ...
            for( const auto& field : st.fields ) {
               if( vo.contains( string(field.name).c_str() ) ) {
                  if( missing_extension )
                     ENU_THROW( pack_exception, "Unexpected '${f}' in variant object", ("f",field.name) );
                  ...
                  ENU_THROW( pack_exception, "Missing '${f}' in variant object", ("f",field.name) );
               }
            }
         } else if( var.is_array() ) {
            const auto& va = var.get_array();
            ENU_ASSERT( st.base == type_name(), invalid_type_inside_abi, "support for base class as array not yet implemented" );
            uint32_t i = 0;
            for( const auto& field : st.fields ) {
               if( va.size() > i )
                  ...
                  ENU_THROW( pack_exception, "Early end to array specifying the fields of struct '${t}'; require input for field '${f}'",
                             ("t", st.name)("f", field.name) );
               ++i;
            }
         } else {
            ENU_THROW( pack_exception, "Failed to serialize struct '${t}' in variant object", ("t", st.name));
         }
      }
