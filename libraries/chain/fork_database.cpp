...
namespace enumivo { namespace chain {
   ...
   void fork_database::set( block_state_ptr s ) {
      ...
      ENU_ASSERT( s->id == s->header.id(), fork_database_exception, 
      ...
      ENU_ASSERT( result.second, fork_database_exception, "unable to insert block state, duplicate state detected" );
      if( !my->head ) {
         my->head =  s;
   ...
   block_state_ptr fork_database::add( block_state_ptr n ) {
      ...
      ENU_ASSERT( inserted.second, fork_database_exception, "duplicate block added?" );
   ...
   block_state_ptr fork_database::add( signed_block_ptr b, bool trust ) {
      ENU_ASSERT( b, fork_database_exception, "attempt to add null block" );
      ...
      ENU_ASSERT( existing == by_id_idx.end(), fork_database_exception, "we already know about this block" );

      auto prior = by_id_idx.find( b->previous );
      ENU_ASSERT( prior != by_id_idx.end(), unlinkable_block_exception, "unlinkable block", ("id", string(b->id()))("previous", string(b->previous)) );

      auto result = std::make_shared<block_state>( **prior, move(b), trust );
      ENU_ASSERT( result, fork_database_exception , "fail to add new block state" );
      return add(result);
   }
   ...
   pair< branch_type, branch_type >  fork_database::fetch_branch_from( const block_id_type& first,
      ...
      while( first_branch->block_num > second_branch->block_num )
      {
         ...
         ENU_ASSERT( first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(first_branch->header.previous)) );
      }
      ...
      while( second_branch->block_num > first_branch->block_num )
      {
         ...
         ENU_ASSERT( second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(second_branch->header.previous)) );
      }
      ...
      while( first_branch->header.previous != second_branch->header.previous )
      {
         ...
         first_branch = get_block( first_branch->header.previous );
         second_branch = get_block( second_branch->header.previous );
         ENU_ASSERT( first_branch && second_branch, fork_db_block_not_found, 
   ...
   void fork_database::mark_in_current_chain( const block_state_ptr& h, bool in_current_chain ) {
      ...
      ENU_ASSERT( itr != by_id_idx.end(), fork_db_block_not_found, "could not find block in fork database" );
   ...
   void fork_database::add( const header_confirmation& c ) {
      ...
      ENU_ASSERT( b, fork_db_block_not_found, "unable to find block id ${id}", ("id",c.block_id));
