...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of ENU to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        ENU_ASSERT( !wasm.empty(), wast_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   sudo_exec->set_callback([&] {
      ...
      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{executer, config::active_name}, {"enu.sudo", config::active_name}};
      }
      ...
      send_actions({chain::action{accountPermissions, "enu.sudo", "exec", variant_to_bin( N(enu.sudo), N(exec), args ) }});
   });
