...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if (vmap.count("specific-num")) {
    ...
    const auto specific_args = vmap["specific-enunode"].as<vector<string>>();
    if (specific_nums.size() != specific_args.size()) {
      cerr << "ERROR: every specific-num argument must be paired with a specific-enunode argument" << endl;
      exit (-1);
    }
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_enunode_args[num] = specific_args[i];
    }
  }
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable ENUMIVO_HOME is set to the build path." << endl;
    exit (-1);
  }
...
launcher_def::init_genesis () {
   ...
   fc::json::from_file(genesis_path).as<enumivo::chain::genesis_state>(genesis_from_file);
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.enumivo_home + "; ";
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      do_command(host, node.name,
                 { { "ENUMIVO_HOME", host.enumivo_home }, { "ENUMIVO_NODE", node_num }, { "ENUMIVO_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/enumivo_tn_roll.sh ";
      do_command(*host, host_name, { { "ENUMIVO_HOME", host->enumivo_home } }, cmd);
   }
}
