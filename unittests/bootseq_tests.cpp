...
public:
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "enumivo_global_state", data, abi_serializer_max_time );
   }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // set_code_abi(N(enu.msig), contracts::enumivo_msig_wasm(), contracts::enumivo_msig_abi().data());//, &enumivo_active_pk);
        // set_code_abi(N(enu.token), contracts::enumivo_token_wasm(), contracts::enumivo_token_abi().data()); //, &enumivo_active_pk);
        ...
        const auto& enumivo_msig_acc = get<account_object, by_name>(N(enu.msig));
        ...
        return;
        produce_blocks(7000); /// produce blocks until virtual bandwidth can acomadate a small user
        wlog("minow" );
