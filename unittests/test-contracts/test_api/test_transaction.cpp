...
struct test_action_action {
   static enumivo::name get_account() {
      return enumivo::name{ACCOUNT};
   }

   static enumivo::name get_name() {
      return enumivo::name{NAME};
   }
...
void test_transaction::assert_false_error_handler( const enumivo::transaction& dtrx ) {
   ...
   enumivo_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   enumivo_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   enumivo_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   enumivo_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   enumivo_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( enumivo::name{"testtrans"}.value, enumivo::name{"table"}.value, enumivo::name{"testtrans"}.value, 0, buf, 4 );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   trx.send( sender_id, enumivo::name{receiver} );
}
