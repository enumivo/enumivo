...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(enumivo_contract_abi(abi), max_serialization_time);
   };
}
...
{ try {
   ...
   abi_serializer abis(enumivo_contract_abi(abi), max_serialization_time);
...
struct abi_gen_helper {
  ...
  bool generate_abi(const char* source, const char* abi, bool opt_sfs=false) {
    ...
    abi_def output;
    output.version = "enumivo::abi/1.0";
...
{ try {
   ...
   abi_serializer abis(enumivo_contract_abi(abi_def()), max_serialization_time);
...
{ try {
   ...
   abi_serializer abis(enumivo_contract_abi(v.as<abi_def>()), max_serialization_time);
...
BOOST_AUTO_TEST_CASE(abi_large_array)
{
   try {
      ...
      // curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json -X POST -d '{"code":"enumivo", "action":"hi", "binargs":"ffffffff08"}'
...
{
   using enumivo::testing::fc_exception_message_starts_with;

   auto duplicate_variant_abi = R"({
      "version": "enumivo::abi/1.1",
      "variants": [
...
BOOST_AUTO_TEST_CASE(version)
{
   try {
      ...
      BOOST_CHECK_THROW( abi_serializer(fc::json::from_string(R"({"version": "enumivo::abi/9.0"})").as<abi_def>(), max_serialization_time), unsupported_abi_version_exception );
      abi_serializer(fc::json::from_string(R"({"version": "enumivo::abi/1.0"})").as<abi_def>(), max_serialization_time);
      abi_serializer(fc::json::from_string(R"({"version": "enumivo::abi/1.1"})").as<abi_def>(), max_serialization_time);
   } FC_LOG_AND_RETHROW()
}
...
{
   using enumivo::testing::fc_exception_message_is;
