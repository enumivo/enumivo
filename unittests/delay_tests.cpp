...
const std::string enumivo_token = name(N(eosio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.set_code(N(eosio.token), enumivo_token_wast);
   chain.set_abi(N(eosio.token), enumivo_token_abi);
