...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   ...
   test.chain->set_code(N(bob), contracts::enumivo_token_wasm() );
   test.chain->set_abi(N(bob), contracts::enumivo_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::enumivo_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::enumivo_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( blacklist_enumivo ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, contracts::enumivo_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
