...
class currency_tester : public TESTER {
   public:
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::enumivo_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(enu.token));
         set_code( N(enu.token), contracts::enumivo_token_wasm() );
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "ENU");
      BOOST_REQUIRE_EQUAL(SY(4,ENU), sys.value());
      BOOST_REQUIRE_EQUAL("4,ENU", sys.to_string());
      BOOST_REQUIRE_EQUAL("ENU", sys.name());
   ...
   // invalid - contains lower case characters, no validation
   {
      BOOST_CHECK_EXCEPTION(symbol malformed(SY(6,EnU)),
                            symbol_type_exception, fc_exception_message_is("invalid symbol: EnU"));
   }
