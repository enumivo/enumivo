...
#include <enu.sudo/enu.sudo.wast.hpp>
#include <enu.sudo/enu.sudo.abi.hpp>
...
class enu_sudo_tester : public tester {
public:
   ...
   enu_sudo_tester() {
      ...
      set_code( N(enu.sudo), enu_sudo_wast, &system_private_key );
      set_abi( N(enu.sudo), enu_sudo_abi, &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(enu.sudo) );
      abi_def abi;
...
transaction enu_sudo_tester::sudo_exec( account_name executer, const transaction& trx, uint32_t expiration ) {
   fc::variants v;
...
transaction enu_sudo_tester::reqauth( account_name from, const vector<permission_level>& auths, uint32_t expiration ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(enu_sudo_tests)

BOOST_FIXTURE_TEST_CASE( sudo_exec_direct, enu_sudo_tester ) try {
...

   {
      ...
      sudo_trx.actions.emplace_back( get_action( N(enu.sudo), N(exec),
                                                 {{N(alice), config::active_name}, {N(enu.sudo), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig, enu_sudo_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   ...
   BOOST_REQUIRE_EQUAL( "enu.sudo", name{traces[0]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig_unapprove, enu_sudo_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   auto sudo_trx = sudo_exec( N(alice), trx );
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of enu.sudo::exec.
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig_producers_change, enu_sudo_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of enu.sudo::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(enu.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
