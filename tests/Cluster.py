...
class Cluster(object):
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
               , dontBootstrap=False, totalProducers=None, extraEnunodeArgs=None, useBiosBootFile=True, specificExtraEnunodeArgs=None):
        """Launch cluster.
        ...
        extraEnunodeArgs: string of arguments to pass through to each nodoes instance (via --enunode flag on launcher)
        ...
        specificExtraEnunodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-enunode flags on launcher), example: { "5" : "--plugin enumivo::test_control_api_plugin" }
        """
        ...
        while not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on enunode default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.EnuLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        enunodeArgs="--max-transaction-time 990000 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
        ...
        if specificExtraEnunodeArgs is not None:
            assert(isinstance(specificExtraEnunodeArgs, dict))
            for nodeNum,arg in specificExtraEnunodeArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-enunode")
                cmdArr.append(arg)
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, dontKill=False):
        """Bootstrap cluster using the bios_boot.sh script generated by enulauncher."""
        ...
        bootlog="enumivo-ignition-wd/bootlog.txt"
    ...
    @staticmethod
    def pgrepEnuServers(timeout=None):
        pgrepOpts="-fl"
    ...
    @staticmethod
    def pgrepEnuServerPattern(nodeInstance):
        if isinstance(nodeInstance, str):
            return r"[\n]?(\d+) (.* --data-dir var/lib/node_%s .*)\n" % nodeInstance
        else:
            nodeInstanceStr="%02d" % nodeInstance
            return Cluster.pgrepEnuServerPattern(nodeInstanceStr)
    ...
    def discoverLocalNodes(self, totalNodes, timeout=None):
        ...
        psOut=Cluster.pgrepEnuServers(timeout)
        if psOut is None:
        ...
        for i in range(0, totalNodes):
            pattern=Cluster.pgrepEnuServerPattern(i)
    ...
    def discoverBiosNodePid(self, timeout=None):
        psOut=Cluster.pgrepEnuServers(timeout=timeout)
        pattern=Cluster.pgrepEnuServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.EnuServerName, pattern))
    ...
    def bounce(self, nodes, silent=True):
        """Bounces enunode instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down enunode instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
