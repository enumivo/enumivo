...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="enumivo-ignition-wd/bootlog.txt"
    __configDir="etc/enumivo/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraEnunodeArgs=None, useBiosBootFile=True, specificExtraEnunodeArgs=None):
        """Launch cluster.
        ...
        enunodeArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by enumivo-launcher."""
    ...
    def pgrepEnuServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.EnuServerName)
