...
try:
    ...
    specificExtraEnunodeArgs={}
    # producer nodes will be mapped to 0 through totalProducerNodes-1, so the number totalProducerNodes will be the non-producing node
    specificExtraEnunodeArgs[totalProducerNodes]="--plugin enumivo::test_control_api_plugin"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, topo="bridge", pnodes=totalProducerNodes,
                      ...
                      useBiosBootFile=False, specificExtraEnunodeArgs=specificExtraEnunodeArgs) is False:
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.enumivoAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.enumivoAccount.name, account.name))
        node.transferFunds(cluster.enumivoAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killEnuInstances=killEnuInstances, killWallet=killWallet, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
