...
try:
    ...
    if testWallet is None:
        Utils.cmdError("enu wallet create")
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.enumivoAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000)
        if trans is None:
        ...
        if node.transferFunds(cluster.enumivoAccount, account, transferAmount, "test transfer") is None:
            errorExit("Failed to transfer funds %d from account %s to %s" % (
                transferAmount, cluster.enumivoAccount.name, account.name))
