...
try:
    ...
    transId=node.createInitializeAccount(currencyAccount, cluster.enumivoAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.enumivoAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.enumivoAccount, waitForTransBlock=True, exitOnError=True)
