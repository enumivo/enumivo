...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.enumivoAccount,cluster.defproduceraAccount])
    ...
    transId=node.createInitializeAccount(testeraAccount, cluster.enumivoAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.enumivoAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.enumivoAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.enumivoAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.enumivoAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
