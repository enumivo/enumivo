...
Execute a transaction while bypassing regular authorization checks (requires authorization of enu.sudo which needs to be a privileged account).

### enu.sudo::exec    executer trx
...
## 2. Installing the enu.sudo contract
...
### 2.1 Create the enu.sudo account
...
The actual public key used is not important since that data is only encoded into the `enumivo::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `enumivo::newaccount` action. It should look like:
```
$ cat newaccount_payload.json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "enumivo", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
Third, generate a transaction containing the actual `enumivo::newaccount` action that will be used in the final transaction:
```
$ enucli push action -s -j -d enumivo newaccount newaccount_payload.json -p enumivo > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `enumivo::setpriv` action which will make the `enu.sudo` account privileged:
```
$ enucli push action -s -j -d enumivo setpriv '{"account": "enu.sudo", "is_priv": 1}' -p enumivo > generated_setpriv_trx.json
...
#### 2.1.2 Propose the transaction to create the enu.sudo account
...
#### 2.1.3 Review and approve the transaction to create the enu.sudo account
...
```
$ enucli multisig review blkproducera createsudo > create_sudo_account_trx_to_review.json
...
```
$ enucli multisig propose_trx -j -s -d createsudo '[]' create_sudo_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_sudo_trx_serialized.hex
...
Anyone can now verify that the `enu.sudo` was created:
```
$ enucli get account enu.sudo
privileged: true
permissions:
     owner     1:    1 enumivo@active,
        active     1:    1 enumivo@active,
memory:
...
### 2.2 Deploy the enu.sudo contract
...
```
$ enucli set contract -s -j -d enu.sudo contracts/enu.sudo/ > deploy_sudo_contract_trx.json
Reading WAST/WASM from contracts/enu.sudo/enu.sudo.wasm...
...
#### 2.2.2 Propose the transaction to deploy the enu.sudo contract
...
#### 2.2.3 Review and approve the transaction to deploy the enu.sudo contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `enucli multisig review` command:
```
$ enucli multisig review blkproducera deploysudo > deploy_sudo_contract_trx_to_review.json
...
```
$ enucli multisig propose_trx -j -s -d deploysudo '[]' deploy_sudo_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_sudo_trx_serialized.hex
...
```
$ enucli get code -a retrieved-enu.sudo.abi enu.sudo
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-enu.sudo.abi
$ sha256sum contracts/enu.sudo/enu.sudo.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/enu.sudo/enu.sudo.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-enu.sudo.abi, can then be compared to the original ABI of the contract (contracts/enu.sudo/enu.sudo.abi) to ensure they are semantically the same.

## 3. Using the enu.sudo contract
...
```
permissions:
     owner     1:    1 ENU6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
        active     1:    1 ENU6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
memory:
...
Assume that none of the block producers know the private key corresponding to the public key `ENU6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV` which, as can be seen above, is initially securing access to the `alice` account.
...
```
$ enucli set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "enumivo", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `enu.sudo::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ enucli sudo exec -s -j -d blkproducera update_alice_owner_trx.json > sudo_update_alice_owner_trx.json
```
...
If the `enucli sudo` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the sudo_update_alice_owner_trx.json file was already generated with content similar to the above using the `enucli sudo exec` sub-command method.
...
```
$ enucli multisig propose_trx -s -j -d nothing '[]' update_alice_owner_trx.json nothing | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > update_alice_owner_trx_serialized.hex
...
Then generate the template for the transaction containing the `enu.sudo::exec` action:
```
$ enucli push action -s -j -d enu.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ enucli multisig review blkproducera updatealice > sudo_update_alice_owner_trx_to_review.json
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of enunode/enucli, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `enu.sudo::exec` action is available in the output of the `enucli multisig review` command, only the hex data is available of the payload of the inner `enumivo::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in enunode/enucli, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `enumivo::setcode` action to set the contract code of the `enu.sudo` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
```
$ enucli multisig propose_trx -j -s -d updatealice '[]' sudo_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_sudo_update_alice_owner_trx_serialized.hex
